...
- The GPS map dot uses a separate `livePosition` state (`navigator.geolocation.watchPosition`) instead of the one-time `getCurrentPosition` fix used for `homeLat`/`homeLng` — otherwise it looked stale while actually driving with the app open. Deliberately not wired into the fetch effect's dependencies, so continuous position updates never trigger repeated station refetches; the dot is purely visual until the user explicitly taps ◎ or "Buscar en esta zona". Tapping ◎ snaps `homeLat`/`homeLng` to this live position (not the original one-time fix) and does so instantly, since the value is already on hand instead of requiring a fresh GPS round-trip
- Follow mode (`Map.tsx`): once GPS is granted, the camera continuously tracks the live dot (checking prices mid-drive shouldn't require repeatedly re-tapping ◎ just to keep your position in view) via `FollowTracker`'s `panTo`. Disengages the instant the user manually drags the map — detected via Leaflet's `dragstart` event specifically, since that only fires for real user-initiated panning and never for the follow mode's own programmatic `panTo` calls, unlike the distance-threshold comparisons `atHome`/`SearchHereButton` rely on. Re-engages on any later tap of ◎
